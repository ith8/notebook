...
getChoice :: IO Player
getChoice = do putStr "Play as X or O? \n"
	       c <- getChar 
	       if elem (toUpper c) ['X','O'] then
	          if c == 'X' then 
		     return X
		  else
		     return O
	       else
	          do putStr "Error: Player must be either X or O \n"
		     getChoice
...
play' g p c
   ...
   | p == c = do i <- getNat (prompt c)
		 case move g i p of
		     [] -> do putStrLn "ERROR: Invalid move"
			      play' g p c
                     [g'] -> play g' (next p) c
   | p /= c = do putStr ("Player " ++ (show (next c)) ++ " is thinking... ")
		 (play $! (bestmove g p)) (next p) c -- $! operator guarantees bestmove evaluates before play

-- solution for part b, c, d in progress
