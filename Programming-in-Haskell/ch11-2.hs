...
bestmove :: Grid -> Player -> [Grid] -- modified to list of best moves
   ...
   | p == O = do i <- getNat (prompt p)
		 case move g i p of
		     [] -> do putStrLn "ERROR: Invalid move"
			      play' g p
                     [g'] -> play g' (next p)
   | p == X = do putStr "Player X is thinking... " -- modified lines
	         let bm = bestmove g p
		 r <- randomRIO (0, (length bm) -1)
		 (play $! (bm !! r)) (next p) -- $! operator guarantees bestmove evaluates before play
